...
me.loadTemplate = function (tplPath) {
  if(me.isTemplate(tplPath)) {
    var tpl = {};
    
    tpl.config = _loadConfig(_getConfigPath(tplPath));
    _.extend(tpl, me._loadTemplateHooks(tplPath + '/process.js'));
 ...
 */
me.isTemplate = function (tplPath) {
  var configFilePath = _getConfigPath(tplPath);
  return fsUtil.isFile(configFilePath) && me._isValidConfig(_loadConfig(configFilePath));
}

me._loadTemplateHooks = function (tplProcessPath) {
  var hooks = {};
...
/**
 * Parses the config.yml file located in the path and returns the content.
 */
function _loadConfig(tplConfigPath) {
  return yaml.safeLoad(fs.readFileSync(tplConfigPath, 'utf8'));
}
