 ...
 * @param skipQuestions  (boolean) pass true when you want to skip questions and use defaults instead
 */
function createProject(templateFolder, templateName, skipQuestions) {
  var path = templateFolder + '/' + templateName;

  if (!templ.isTemplate(path)) {
    throw new Error('The given template is not valid. Check the path: "' + path + '"');
  }

  var variablesNeeded = templ.getRequiredVariables(templateFolder, path);
  variablesNeeded.push('name'); //name is always needed for folder name
