...
module.exports = initTemplatesDirectory;
...
/**
 * Initializes a templates directory in the user's home directory
 */
function initTemplatesDirectory(folder) {
    ...
    if (answer.continue) {
      ...
      inquirer.prompt(sampleQuestion, function(answer) {
        if (answer.createSample) {
          sampleData.createTemplates(folder, printCreatedMsg);
          console.log('');
        }
      });
    }
